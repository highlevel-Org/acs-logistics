...
function Logout() {
	...
	return (
		<div className='container'>
			<motion.div className={NavClass}  animate={{width:OpenNav? '17%': '0%'}}>
			...
			{OpenNav && <h2>
				...
				{ShowNav && <div className='navControl'>
					{OpenNav ? (
				...
				{OpenNav && <div className='navList'>
					{sidebarData.map((nav, index) => {
						return (
							<Link  to={nav.link} key={index} className='navListItem'  id={Location == nav.link ? "active":""}>
								<span >{nav.icon}</span>
								<p>{nav.title}</p>
							</Link>
						);
					...
					//  id={Location == nav.link ? "active":""}
					 ...
					 >
								<span ><ExitToAppIcon  sx={{fontSize:30, color:'white'}}/></span>
								<p>Logout</p>
							</div>
				</div>}
			</motion.div>
